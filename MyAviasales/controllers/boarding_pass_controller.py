...
class BoardingPassController(BaseController):
    async def get_all_boarding_passes(self):
        boarding_passes = await self.session.query(
    ...
    async def get_single_boarding_pass(self, data):
        boarding_pass = await self.session.query(
    ...
    async def post_boarding_pass(self, data):
        await self.session.add(BoardingPass(
    ...
    async def delete_boarding_pass(self, data: dict):
        ...
        await self.session.delete(boarding_pass)  # try/catch?
        await self.session.commit()
        return True

    async def put_ticket(self, data):  # TODO: Проверить, что работает
        boarding_pass = await self.session.query(BoardingPass).filter(
        ...
        await self.session.add(boarding_pass)
