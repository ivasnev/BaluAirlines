...
class BookingController(BaseController):
    async def get_all_bookings(self, page: int) -> Optional[List[BookingResponse]]:
        page_size = 50
        bookings_query = await self.session.query(Booking.book_ref,
                                            Booking.book_date,
        ...
        for booking in bookings:
            booking.tickets = await TicketController(await self.session) \
                .get_all_ticket_by_book_ref(booking.book_ref)
        return bookings

    async def get_single_booking(self, key: str) -> Optional[BookingResponse]:
        booking = await self.session.query(Booking.book_ref,
                                     Booking.book_date,
                                     Booking.total_amount).filter_by(
            book_ref=key).first()
        if booking:
            booking = BookingResponse.from_orm(booking)
            booking.tickets = await TicketController(await self.session).get_all_ticket_by_book_ref(key)
        return booking
    ...
    async def post_booking(self, data: BookingPostRequest) -> bool:
        ...
        for flight_id in data.flights:
            flight = await self.session.query(Flight.departure_airport,
                                        Flight.arrival_airport
                                        ).get(flight_id).one_or_none()
            departure_airport = await self.session.query(AirportsDatum.coordinates).filter(
                AirportsDatum.airport_code == flight.departure_airport).first()[0]
            arrival_airport = await self.session.query(AirportsDatum.coordinates).filter(
            ...
            for ticket_no in tickets:
                ...
                total_amount += amount
            await self.session.flush()
        await self.session.add(Booking(book_ref=book_ref,
    ...
    async def delete_booking(self, key: str) -> bool:
        booking = await self.session.query(Booking).get(key).one_or_none()
        if booking is None:
            return False
        tickets = await self.session.query(Ticket).get(book_ref=key).all()
        for ticket in tickets:
            ticket_flight = await self.session.query(TicketFlight).get(ticket.ticket_no).all()
            await self.session.delete(ticket_flight)
            await self.session.flush()
        await self.session.delete(tickets)
        await self.session.flush()
    ...
    async def put_booking(self, book_ref: str, data: BookingUpdate) -> bool:
        ...
        # obj_to_update = await self.session.query(Booking).get(book_ref).one_or_none()
